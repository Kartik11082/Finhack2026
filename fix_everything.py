...
TICKERS = ["NVDA", "AMD", "MSFT", "GOOGL", "META", "INTC", "PLTR", "AMZN", "CRM"]
...
def evaluate_model(X: pd.DataFrame, y: pd.Series, model_name: str) -> pd.DataFrame:
    ...
    if not results.empty:
        print(f"  Mean: acc={results['accuracy'].mean():.3f} "
...
def main():
    ...
    # Clean NaNs and infs
    check_cols = BASELINE_FEATURES + SENTIMENT_FEATURES + TRANSCRIPT_FEATURES + [LABEL_COL]
    ...
    res_base = evaluate_model(df_base[BASELINE_FEATURES], df_base[LABEL_COL], "Baseline")
    res_enh = evaluate_model(df_enh[ENHANCED_FEATURES], df_enh[LABEL_COL], "Enhanced")

    # ------------------------------------------------------------------
    # Step 7 — Train final models
    # ------------------------------------------------------------------
    clf_base, _ = train_final_model(df_base[BASELINE_FEATURES], df_base[LABEL_COL], "Baseline", "baseline")
    clf_enh, scaler_enh = train_final_model(df_enh[ENHANCED_FEATURES], df_enh[LABEL_COL], "Enhanced", "enhanced")
    ...
    print("=== FINAL RESULTS ===")
    print("=" * 60)
    print(f"Baseline  ({comp['baseline']['n_rows']} rows, {comp['baseline']['n_tickers']} tickers): "
          f"acc={comp['baseline']['accuracy']} "
          f"f1={comp['baseline']['f1']} auc={comp['baseline']['auc']}")
    print(f"Enhanced  ({comp['enhanced']['n_rows']} rows, {comp['enhanced']['n_tickers']} tickers): "
    ...
    print(f"Delta: acc={'+' if delta_acc > 0 else ''}{delta_acc} "
